...
package Database_Operations is
   Database_Error, Record_Not_Found, Duplicate_Record, Invalid_Input : exception;
   ...
   -- (All other CRUD procedure declarations remain the same)
   procedure Add_Airport(Name : String; Location : String; Max_Capacity : Positive);
   function List_Airports return Flight_Types.Airport_Vectors.Vector;
   procedure Update_Airport(Old_Name, New_Name, New_Location : String; New_Capacity : Positive);
   ...
   procedure Update_Controller(Old_License, New_Name : String; New_Experience : Natural);
   procedure Delete_Controller(By_License : String);

   procedure Add_Flight(Identifier, Origin_Airport_Name, Dest_Airport_Name : String);
   function List_Flights return Flight_Types.Flight_Vectors.Vector;
   procedure Update_Flight(Old_Identifier, New_Origin_Name, New_Dest_Name : String);
